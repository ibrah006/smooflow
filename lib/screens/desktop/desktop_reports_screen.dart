...
import 'package:smooflow/screens/desktop/components/reports_page_skeleton.dart';
...
_ReportsData _computeReports({
  ...
  final projects =
      projTotals.entries.map((e) {
          final proj = allProjects.cast<Project?>().firstWhere(
            (p) => p?.id == e.key,
            orElse: () => null,
          );
  ...
  for (final t in txns) {
    ...
    final client =
        (proj?.client.name.isNotEmpty == true)
            ? proj!.client.name
            : 'Internal / Unassigned';
...
class _MaterialReportsScreenState extends ConsumerState<DesktopReportsScreen> {
      ...
      children: [
        ...
        Column(
          ...
          children: [
            ...
            const Text(
              'Consumption analysis for your print team',
              style: TextStyle(
                fontSize: 10.5,
                color: _T.slate400,
    ...
    child: PopupMenuButton<T>(
      ...
      child: Padding(
        ...
        child: Row(
          mainAxisSize: MainAxisSize.min,
          children: [
            Icon(icon, size: 13, color: hasReset ? accentColor : _T.slate400),
...
class _KpiRow extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    final avgPerJob =
        data.totalJobs > 0 ? data.totalConsumed / data.totalJobs : 0.0;
...
class _MonthlyTrendChartState extends State<_MonthlyTrendChart> {
  ...
  void _buildSeries() {
    ...
    List<double> valuesFor(String? id) =>
        widget.months.map((m) {
          if (id == null) {
              ...
              Expanded(
                child: LayoutBuilder(
                  builder: (_, constraints) {
                    ...
                    return MouseRegion(
                      onHover:
                          (e) => _onHover(e.localPosition, canvasW, canvasH),
  ...
  void paint(Canvas canvas, Size size) {
    ...
    for (final s in series) {
      ...
      for (int i = 0; i < pts.length; i++) {
        ...
        final p = pts[i];
        final isHovered =
            hovered?.seriesId == s.id && (hovered!.dx - p.dx).abs() < 1.0;
        final radius = isHovered ? 5.5 : 3.5;

        canvas.drawCircle(p, radius, Paint()..color = s.color);
        canvas.drawCircle(
          p,
          radius,
          Paint()
...
class _ProjectBars extends StatefulWidget {
  ...
  @override
  State<_ProjectBars> createState() => _ProjectBarsState();
}
...
class _ProjectBarsState extends State<_ProjectBars> {
  ...
  Widget build(BuildContext context) {
    if (widget.projects.isEmpty)
      return _noData('No project data in this period');
    final maxVal = widget.projects
        .map((p) => p.totalConsumed)
        .fold(0.0, math.max);

    return Column(
      children:
          widget.projects.take(10).map((p) {
                                  ...
                                  (_, c) => Stack(
                                    children: [
                                      ...
                                      AnimatedContainer(
                                        ...
                                        decoration: BoxDecoration(
                                          color: p.projectColor.withOpacity(
                                            0.75,
                                          ),
                                          borderRadius: BorderRadius.circular(
                                            5,
                                  ...
                                  (e) => Padding(
                                    ...
                                    child: Row(
                                      children: [
                                        ...
                                        Text(
                                          '${_fmtQty(e.value)} ${widget.matUnit(e.key)}',
                                          style: const TextStyle(
                                            fontSize: 11,
                                            fontWeight: FontWeight.w700,
                                            color: _T.ink3,
...
class _ClientBars extends StatefulWidget {
  ...
  @override
  State<_ClientBars> createState() => _ClientBarsState();
}
...
class _ClientBarsState extends State<_ClientBars> {
  ...
  Widget build(BuildContext context) {
    ...
    final maxVal = widget.clients
        .map((c) => c.totalConsumed)
        .fold(0.0, math.max);

    return Column(
      children:
          widget.clients.take(10).toList().asMap().entries.map((entry) {
            final i = entry.key;
...
class _StockHealthChart extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    final sorted = List<MaterialModel>.from(allMats)..sort((a, b) {
                ...
                children: [
                  Row(
                    children: [
                      ...
                      Text(
                        '${_fmtQty(m.currentStock)} / ${_fmtQty(m.minStockLevel)} ${m.unitShort}',
                        style: const TextStyle(
                          fontSize: 10.5,
                          fontWeight: FontWeight.w600,
                          color: _T.slate500,
...
class _TopJobsTable extends ConsumerWidget {
  ...
  String projectName(WidgetRef ref, String projectId) =>
      ref.watch(projectByIdProvider(projectId))!.name;
              ...
              children: [
                ...
                Expanded(
                  ...
                  child: Row(
                    ...
                    children: [
                      Text(
                        '${_fmtQty(t.quantity)} ${matUnit(t.materialId)}',
                        style: TextStyle(
                          fontSize: 12.5,
                          fontWeight: FontWeight.w600,
                        ),
  ...
  Widget build(BuildContext context) => Center(
    child: Column(
      ...
      children: [
        ...
        const SizedBox(height: 6),
        Text(
          'No stock-out events recorded in ${period.label.toLowerCase()}.',
          style: const TextStyle(fontSize: 12.5, color: _T.slate400),
        ),
        const SizedBox(height: 4),
        const Text(
          'Start print jobs to see consumption analytics.',
